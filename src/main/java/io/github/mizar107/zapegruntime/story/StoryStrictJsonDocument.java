...
/** Bounded JSON reader which rejects duplicate object keys. */
final class StoryStrictJsonDocument {

    private static final int MAX_DEPTH = 16;
    private static final int MAX_VALUES = 2_048;
    private static final int MAX_STRING_LENGTH = 256;

    private StoryStrictJsonDocument() {}
    ...
    static JsonElement parse(Reader source) {
        try {
            ...
            throw new JsonParseException("invalid strict story JSON", invalid);
        }
    }
    ...
    private static JsonElement readValue(JsonReader reader, int depth, Counter counter)
            throws IOException {
        if (depth > MAX_DEPTH) {
            throw new JsonParseException("story JSON exceeds maximum nesting depth");
        }
        counter.increment();
        return switch (reader.peek()) {
            case BEGIN_OBJECT -> readObject(reader, depth + 1, counter);
            case BEGIN_ARRAY -> readArray(reader, depth + 1, counter);
            case STRING -> new JsonPrimitive(readBoundedString(reader, "string"));
    ...
    private static String readBoundedString(JsonReader reader, String description)
            throws IOException {
        String value = reader.nextString();
        if (value.length() > MAX_STRING_LENGTH) {
            throw new JsonParseException(description + " exceeds maximum length");
        }
        return value;
    }
    ...
    private static final class Counter {
        ...
        private void increment() {
            ...
            if (values > MAX_VALUES) {
                throw new JsonParseException("story JSON exceeds maximum value count");
            }
        }
    }
}
